...
export async function sendProactive(
  ...
  try {
    ...
    if (type === "c2c") {
      result = await sendProactiveC2CMessage(accessToken, to, text, account.config?.proxyUrl);
    } else if (type === "group") {
      result = await sendProactiveGroupMessage(accessToken, to, text, account.config?.proxyUrl);
