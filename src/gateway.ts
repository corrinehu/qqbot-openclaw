...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  const connect = async () => {
    ...
    try {
      ...
      const accessToken = await getAccessToken(account.appId, account.clientSecret, account.config?.proxyUrl);
      const gatewayUrl = await getGatewayUrl(accessToken, account.config?.proxyUrl);

      log?.info(`[qqbot:${account.accountId}] Connecting to ${gatewayUrl}`);

      const agent = account.config?.proxyUrl && account.config.proxyUrl.startsWith("socks")
        ? new SocksProxyAgent(account.config.proxyUrl)
        : undefined;
...
请根据图片内容进行回复。`);
        ...
        const sendWithTokenRetry = async (sendFn: (token: string) => Promise<unknown>) => {
          try {
            const token = await getAccessToken(account.appId, account.clientSecret, account.config?.proxyUrl);
            await sendFn(token);
          } catch (err) {
            ...
            if (errMsg.includes("401") || errMsg.includes("token") || errMsg.includes("access_token")) {
              ...
              clearTokenCache();
              const newToken = await getAccessToken(account.appId, account.clientSecret, account.config?.proxyUrl);
      ...
      ws.on("open", () => {
        ...
        // P1-1: 启动后台 Token 刷新
        startBackgroundTokenRefresh(account.appId, account.clientSecret, account.config?.proxyUrl, {
